import { a as C, d as V } from "@nf-internal/chunk-EMAF5KOL";
import "@nf-internal/chunk-XUYTQVBT";
import { a as w } from "@nf-internal/chunk-MBDOAHVM";
import { c as T } from "@nf-internal/chunk-3C63DHR6";
import { b as H } from "@nf-internal/chunk-ZBGE3AVM";
import { c as k } from "@nf-internal/chunk-C2A5IV4Z";
import { D as E, H as N, i as D, j as y, n as F, w as _ } from "@nf-internal/chunk-IR6MR6UK";
import "@nf-internal/chunk-N26NRATF";
import { a as P, b as O } from "@nf-internal/chunk-6MDQTQU3";
...
var b = class {
    ...
    getDescendants(o) {
        let e = this.dataNodes.indexOf(o), t = [];
        for (let n = e + 1; n < this.dataNodes.length && this.getLevel(o) < this.getLevel(this.dataNodes[n]); n++)
            t.push(this.dataNodes[n]);
        return t;
    }
    ...
    _getDescendants(o, e) {
        ...
        Array.isArray(t) ? t.forEach(n => this._getDescendants(o, n)) : k(t) && t.pipe(x(1), Z(Boolean)).subscribe(n => {
            for (let s of n)
                this._getDescendants(o, s);
        });
    }
}, G = new ee("CDK_TREE_NODE_OUTLET_NODE"), K = (() => {
    class i {
        viewContainer = h(te);
    ...
    class i {
        ...
        _subscribeToDataChanges() {
            if (this._dataSubscription)
                return;
            let e;
            H(this._dataSource) ? e = this._dataSource.connect(this) : k(this._dataSource) ? e = this._dataSource : Array.isArray(this._dataSource) && (e = _(this._dataSource)), e && (this._dataSubscription = this._getRenderData(e).pipe(p(this._onDestroy)).subscribe(t => { this._renderDataChanges(t); }));
        }
        ...
        _emitExpansionChanges(e) {
            if (!e)
                return;
            let t = this._nodes.value;
            for (let n of e.added)
                t.get(n)?._emitExpansionState(!0);
            for (let n of e.removed)
        ...
        renderNodeChanges(e, t = this._dataDiffer, n = this._nodeOutlet.viewContainer, s) {
            let r = t.diff(e);
            !r && !this._viewInit || (r?.forEachOperation((l, d, c) => {
                if (l.previousIndex == null)
        ...
        _getNodeDef(e, t) {
            ...
            let n = this._nodeDefs.find(s => s.when && s.when(t, e)) || this._defaultNodeDef;
            return n;
        }
        ...
        _getDirectChildren(e) {
            let t = this._getLevelAccessor(), n = this._expansionModel ?? this.treeControl?.expansionModel;
            if (!n)
                return _([]);
            let s = this._getExpansionKey(e), r = n.changed.pipe(M(d => d.added.includes(s) ? _(!0) : d.removed.includes(s) ? _(!1) : F), U(this.isExpanded(e)));
            if (t)
                return E([r, this._flattenedNodes]).pipe(u(([d, c]) => d ? this._findChildrenByLevel(t, c, e, 1) : []));
            let l = this._getChildrenAccessor();
            if (l)
                return C(l(e) ?? []);
            throw q();
        }
        _findChildrenByLevel(e, t, n, s) {
            let r = this._getExpansionKey(n), l = t.findIndex(g => this._getExpansionKey(g) === r), d = e(n), c = d + s, f = [];
        ...
        _getDescendants(e) {
            if (this.treeControl)
                return _(this.treeControl.getDescendants(e));
            if (this.levelAccessor) {
                let t = this._findChildrenByLevel(this.levelAccessor, this._flattenedNodes.value, e, 1 / 0);
                return _(t);
            }
            if (this.childrenAccessor)
                return this._getAllChildrenRecursively(e).pipe(Q((t, n) => (t.push(...n), t), []));
            throw q();
        }
        _getAllChildrenRecursively(e) {
            return this.childrenAccessor ? C(this.childrenAccessor(e)).pipe(x(1), M(t => {
                for (let n of t)
                    this._parents.set(this._getExpansionKey(n), e);
                return _(...t).pipe(z(n => N(_([n]), this._getAllChildrenRecursively(n))));
            })) : _([]);
        }
        ...
        _findParentForNode(e, t, n) {
            if (!n.length)
                return null;
            let s = this._levels.get(this._getExpansionKey(e)) ?? 0;
            for (let r = t - 1; r >= 0; r--) {
                let l = n[r];
        ...
        _flattenNestedNodesWithExpansion(e, t = 0) {
            let n = this._getChildrenAccessor();
            return n ? _(...e).pipe(z(s => {
                let r = this._getExpansionKey(s);
                this._parents.has(r) || this._parents.set(r, null), this._levels.set(r, t);
                let l = C(n(s));
                return N(_([s]), l.pipe(x(1), m(d => {
                    this._ariaSets.set(r, [...d ?? []]);
                    for (let c of d ?? []) {
                        let f = this._getExpansionKey(c);
                        this._parents.set(f, s), this._levels.set(f, t + 1);
                    }
                }), M(d => d ? this._flattenNestedNodesWithExpansion(d, t + 1).pipe(u(c => this.isExpanded(s) ? c : [])) : _([]))));
            }), Q((s, r) => (s.push(...r), s), [])) : _([...e]);
        }
        _computeRenderingData(e, t) {
            if (this.childrenAccessor && t === "flat")
                return this._clearPreviousCache(), this._ariaSets.set(null, [...e]), this._flattenNestedNodesWithExpansion(e).pipe(u(n => ({ renderNodes: n, flattenedNodes: n })));
            if (this.levelAccessor && t === "nested") {
        ...
        static \u0275fac = function (t) { return new (t || i); };
        static \u0275cmp = a.\u0275\u0275defineComponent({ type: i, selectors: [["cdk-tree"]], contentQueries: function (t, n, s) {
                if (t & 1 && a.\u0275\u0275contentQuery(s, se, 5), t & 2) {
                    let r;
                    a.\u0275\u0275queryRefresh(r = a.\u0275\u0275loadQuery()) && (n._nodeDefs = r);
                }
            }, viewQuery: function (t, n) {
                if (t & 1 && a.\u0275\u0275viewQuery(K, 7), t & 2) {
                    let s;
...
function ie(i) {
    ...
    for (; o && !re(o);)
        o = o.parentElement;
    return o ? o.classList.contains("cdk-nested-tree-node") ? S(o.getAttribute("aria-level")) : 0 : -1;
}
...
var Te = (() => {
    class i extends v {
        ...
        updateChildrenNodes(e) {
            let t = this._getNodeOutlet();
            if (e && (this._children = e), t && this._children) {
        ...
        static \u0275fac = function (t) { return new (t || i); };
        static \u0275dir = a.\u0275\u0275defineDirective({ type: i, selectors: [["cdk-nested-tree-node"]], contentQueries: function (t, n, s) {
                if (t & 1 && a.\u0275\u0275contentQuery(s, K, 5), t & 2) {
                    let r;
    ...
    class i {
        ...
        _setPadding(e = !1) {
            let t = this._paddingIndent();
            if (t !== this._currentPadding || e) {
