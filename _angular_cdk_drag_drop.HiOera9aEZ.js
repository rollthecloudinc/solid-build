import { f as ut, g as $t, i as ft, p as mt } from "@nf-internal/chunk-RASHQLKI";
...
import "@nf-internal/chunk-IOPJEBHZ";
import "@nf-internal/chunk-ZBGE3AVM";
import { b as lt, g as O } from "@nf-internal/chunk-C2A5IV4Z";
import { J as ht, b as S, d as V, i as p, j as at } from "@nf-internal/chunk-IR6MR6UK";
...
function Z(r) {
    let e = r.cloneNode(!0), t = e.querySelectorAll("[id]"), i = r.nodeName.toLowerCase();
    e.removeAttribute("id");
    for (let s = 0; s < t.length; s++)
        t[s].removeAttribute("id");
    return i === "canvas" ? wt(r, e) : (i === "input" || i === "select" || i === "textarea") && Dt(r, e), vt("canvas", r, e, wt), vt("input, textarea, select", r, e, Dt), e;
}
function vt(r, e, t, i) {
...
var M = class {
    ...
    handleScroll(e) {
        let t = R(e), i = this.positions.get(t);
        if (!i)
            return null;
        let s = i.scrollPosition, n, o;
        if (t === this._document) {
            let h = this.getViewportScrollPosition();
            n = h.top, o = h.left;
        }
        else
            n = t.scrollTop, o = t.scrollLeft;
        let a = s.top - n, l = s.left - o;
        return this.positions.forEach((h, _) => { h.clientRect && t !== _ && t.contains(_) && x(h.clientRect, a, l); }), s.top = n, s.left = o, { top: a, left: l };
    }
    getViewportScrollPosition() { return { top: window.scrollY, left: window.scrollX }; }
};
function Ft(r, e) {
    let t = r.rootNodes;
    if (t.length === 1 && t[0].nodeType === e.ELEMENT_NODE)
        return t[0];
...
function re(r) {
    let e = getComputedStyle(r), t = U(e, "transition-property"), i = t.find(a => a === "transform" || a === "all");
    if (!i)
        return 0;
    let s = t.indexOf(i), n = U(e, "transition-duration"), o = U(e, "transition-delay");
    return yt(n[s]) + yt(o[s]);
}
...
var oe = new Set(["position"]), q = class {
    ...
    _createPreview() {
        let e = this._previewTemplate, t = this._previewClass, i = e ? e.template : null, s;
        if (i && e) {
            ...
            s = Z(this._rootElement), St(s, this._initialDomRect), this._initialTransform && (s.style.transform = this._initialTransform);
        return et(s.style, { "pointer-events": "none", margin: Rt(s) ? "0 auto 0 0" : "0", position: "fixed", top: "0", left: "0", "z-index": this._zIndex + "" }, oe), y(s, !1), s.classList.add("cdk-drag-preview"), s.setAttribute("popover", "manual"), s.setAttribute("dir", this._direction), t && (Array.isArray(t) ? t.forEach(n => s.classList.add(n)) : s.classList.add(t)), s;
    }
};
...
var ae = { passive: !0 }, Et = { passive: !1 }, le = { passive: !1, capture: !0 }, he = 800, It = "cdk-drag-placeholder", xt = new Set(["position"]), $ = class {
    ...
    resetToBoundary() {
        if (this._boundaryElement && this._rootElement && ne(this._boundaryElement.getBoundingClientRect(), this._rootElement.getBoundingClientRect())) {
    ...
    _pointerMove = e => {
        ...
        let i = this._getConstrainedPointerPosition(t);
        if (this._hasMoved = !0, this._lastKnownPointerPosition = t, this._updatePointerDirectionDelta(i), this._dropContainer)
    ...
    _startDragSequence(e) {
        E(e) && (this._lastTouchEventTime = Date.now()), this._toggleNativeDragInteractions();
        let t = this._getShadowRoot(), i = this._dropContainer;
        if (t && this._ngZone.runOutsideAngular(() => { this._cleanupShadowRootSelectStart = this._renderer.listen(t, "selectstart", ce, le); }), i) {
            ...
            this.started.next({ source: this, event: e }), this._initialContainer = this._initialIndex = void 0;
        this._parentPositions.cache(i ? i.getScrollableParents() : []);
    }
    _initializeDragSequence(e, t) {
        this._parentDragRef && t.stopPropagation();
        let i = this.isDragging(), s = E(t), n = !s && t.button !== 0, o = this._rootElement, a = R(t), l = !s && this._lastTouchEventTime && this._lastTouchEventTime + he > Date.now(), h = s ? dt(t) : ct(t);
        if (a && a.draggable && t.type === "mousedown" && t.preventDefault(), i || n || l || h)
            return;
        if (this._handles.length) {
            let D = o.style;
            this._rootElementTapHighlight = D.webkitTapHighlightColor || "", D.webkitTapHighlightColor = "transparent";
        }
        this._hasMoved = !1, this._hasStartedDragging.set(this._hasMoved), this._removeListeners(), this._initialDomRect = this._rootElement.getBoundingClientRect(), this._pointerMoveSubscription = this._dragDropRegistry.pointerMove.subscribe(this._pointerMove), this._pointerUpSubscription = this._dragDropRegistry.pointerUp.subscribe(this._pointerUp), this._scrollSubscription = this._dragDropRegistry.scrolled(this._getShadowRoot()).subscribe(D => this._updateOnScroll(D)), this._boundaryElement && (this._boundaryRect = tt(this._boundaryElement));
        let _ = this._previewTemplate;
        this._pickupPositionInElement = _ && _.template && !_.matchSize ? { x: 0, y: 0 } : this._getPointerPositionInElement(this._initialDomRect, e, t);
        let u = this._pickupPositionOnPage = this._lastKnownPointerPosition = this._getPointerPositionOnPage(t);
        this._pointerDirectionDelta = { x: 0, y: 0 }, this._pointerPositionAtLastDirectionChange = { x: u.x, y: u.y }, this._dragStartTime = Date.now(), this._dragDropRegistry.startDragging(this, t);
    }
    ...
    _animatePreviewToPlaceholder() {
        ...
        let e = this._placeholder.getBoundingClientRect();
        this._preview.addClass("cdk-drag-animating"), this._applyPreviewTransform(e.left, e.top);
        let t = this._preview.getTransitionDuration();
        return t === 0 ? Promise.resolve() : this._ngZone.runOutsideAngular(() => new Promise(i => { let s = a => { (!a || this._preview && R(a) === this._preview.element && a.propertyName === "transform") && (o(), i(), clearTimeout(n)); }, n = setTimeout(s, t * 1.5), o = this._preview.addEventListener("transitionend", s); }));
    }
    ...
    _getPointerPositionOnPage(e) {
        let t = this._getViewportScrollPosition(), i = E(e) ? e.touches[0] || e.changedTouches[0] || { pageX: 0, pageY: 0 } : e, s = i.pageX - t.left, n = i.pageY - t.top;
    ...
    _getConstrainedPointerPosition(e) {
        let t = this._dropContainer ? this._dropContainer.lockAxis : null, { x: i, y: s } = this.constrainPosition ? this.constrainPosition(e, this, this._initialDomRect, this._pickupPositionInElement) : e;
        if (this.lockAxis === "x" || t === "x" ? s = this._pickupPositionOnPage.y - (this.constrainPosition ? this._pickupPositionInElement.y : 0) : (this.lockAxis === "y" || t === "y") && (i = this._pickupPositionOnPage.x - (this.constrainPosition ? this._pickupPositionInElement.x : 0)), this._boundaryRect) {
    ...
    _toggleNativeDragInteractions() {
        if (!this._rootElement || !this._handles)
            return;
        let e = this._handles.length > 0 || !this.isDragging();
        e !== this._nativeInteractionsEnabled && (this._nativeInteractionsEnabled = e, y(this._rootElement, e));
    }
    ...
    _containInsideBoundaryOnResize() {
        ...
        if (e === 0 && t === 0 || this.isDragging() || !this._boundaryElement)
            return;
        let i = this._rootElement.getBoundingClientRect(), s = this._boundaryElement.getBoundingClientRect();
        if (s.width === 0 && s.height === 0 || i.width === 0 && i.height === 0)
            return;
        let n = s.left - i.left, o = i.right - s.right, a = s.top - i.top, l = i.bottom - s.bottom;
        s.width > i.width ? (n > 0 && (e += n), o > 0 && (e -= o)) : e = 0, s.height > i.height ? (a > 0 && (t += a), l > 0 && (t -= l)) : t = 0, (e !== this._passiveTransform.x || t !== this._passiveTransform.y) && this.setFreeDragPosition({ y: t, x: e });
    }
    ...
    _conditionallyInsertAnchor(e, t, i) {
        if (e === this._initialContainer)
...
function Ht(r, e, t) {
    let i = T(e, r.length - 1), s = T(t, r.length - 1);
    if (i === s)
        return;
    let n = r[i], o = s < i ? -1 : 1;
    for (let a = i; a !== s; a += o)
...
var F = class {
    ...
    sort(e, t, i, s) {
        let n = this._itemPositions, o = this._getItemIndexFromPointerPosition(e, t, i, s);
        if (o === -1 && n.length > 0)
            return null;
        let a = this.orientation === "horizontal", l = n.findIndex(f => f.drag === e), h = n[o], _ = n[l].clientRect, u = h.clientRect, D = l > o ? 1 : -1, H = this._getItemOffsetPx(_, u, D), Zt = this._getSiblingOffsetPx(l, n, D), jt = n.slice();
        return Ht(n, l, o), n.forEach((f, qt) => {
            if (jt[qt] === f)
                return;
            let nt = f.drag === e, z = nt ? H : Zt, rt = nt ? e.getPlaceholderElement() : f.drag.getRootElement();
            f.offset += z;
            let ot = Math.round(f.offset * (1 / f.drag.scale));
            a ? (rt.style.transform = A(`translate3d(${ot}px, 0, 0)`, f.initialTransform), x(f.clientRect, 0, z)) : (rt.style.transform = A(`translate3d(0, ${ot}px, 0)`, f.initialTransform), x(f.clientRect, z, 0));
        }), this._previousSwap.overlaps = j(u, t, i), this._previousSwap.drag = h.drag, this._previousSwap.delta = a ? s.x : s.y, { previousIndex: l, currentIndex: o };
    }
    enter(e, t, i, s) {
        let n = s == null || s < 0 ? this._getItemIndexFromPointerPosition(e, t, i) : s, o = this._activeDraggables, a = o.indexOf(e), l = e.getPlaceholderElement(), h = o[n];
        if (h === e && (h = o[n + 1]), !h && (n == null || n === -1 || n < o.length - 1) && this._shouldEnterAsFirstChild(t, i) && (h = o[0]), a > -1 && o.splice(a, 1), h && !this._dragDropRegistry.isDragging(h)) {
            let _ = h.getRootElement();
            _.parentElement.insertBefore(l, _), o.splice(n, 0, e);
        }
        else
            this._element.appendChild(l), o.push(e);
    ...
    _getSiblingOffsetPx(e, t, i) {
        let s = this.orientation === "horizontal", n = t[e].clientRect, o = t[e + i * -1], a = n[s ? "width" : "height"] * i;
        if (o) {
    ...
    _shouldEnterAsFirstChild(e, t) {
        if (!this._activeDraggables.length)
            return !1;
        let i = this._itemPositions, s = this.orientation === "horizontal";
    ...
    _getItemIndexFromPointerPosition(e, t, i, s) {
        let n = this.orientation === "horizontal", o = this._itemPositions.findIndex(({ drag: a, clientRect: l }) => {
            if (a === e)
                return !1;
            if (s) {
                let h = n ? s.x : s.y;
            ...
            return n ? t >= Math.floor(l.left) && t < Math.floor(l.right) : i >= Math.floor(l.top) && i < Math.floor(l.bottom);
        });
        return o === -1 || !this._sortPredicate(o, e) ? -1 : o;
    }
}, K = class {
    _document;
    _dragDropRegistry;
    ...
    start(e) {
        let t = this._element.childNodes;
        this._relatedNodes = [];
    ...
    sort(e, t, i, s) {
        let n = this._getItemIndexFromPointerPosition(e, t, i), o = this._previousSwap;
        if (n === -1 || this._activeItems[n] === e)
            return null;
        let a = this._activeItems[n];
        if (o.drag === a && o.overlaps && o.deltaX === s.x && o.deltaY === s.y)
            return null;
        let l = this.getItemIndex(e), h = e.getPlaceholderElement(), _ = a.getRootElement();
        n > l ? _.after(h) : _.before(h), Ht(this._activeItems, l, n);
        let u = this._getRootNode().elementFromPoint(t, i);
        return o.deltaX = s.x, o.deltaY = s.y, o.drag = a, o.overlaps = _ === u || _.contains(u), { previousIndex: l, currentIndex: n };
    }
    ...
    withSortPredicate(e) { this._sortPredicate = e; }
    reset() {
        let e = this._element, t = this._previousSwap;
    ...
    _getClosestItemIndexToPointer(e, t, i) {
        if (this._activeItems.length === 0)
            return -1;
        if (this._activeItems.length === 1)
            return 0;
        let s = 1 / 0, n = -1;
        for (let o = 0; o < this._activeItems.length; o++) {
    ...
    withElementContainer(e) {
        if (e === this._container)
            return this;
        let t = w(this.element), i = this._scrollableElements.indexOf(this._container), s = this._scrollableElements.indexOf(e);
        return i > -1 && this._scrollableElements.splice(i, 1), s > -1 && this._scrollableElements.splice(s, 1), this._sortStrategy && this._sortStrategy.withElementContainer(e), this._cachedShadowRoot = null, this._scrollableElements.unshift(e), this._container = e, this;
    }
    ...
    _sortItem(e, t, i, s) {
        if (this.sortingDisabled || !this._domRect || !bt(this._domRect, Tt, t, i))
            return;
        let n = this._sortStrategy.sort(e, t, i, s);
        n && this.sorted.next({ previousIndex: n.previousIndex, currentIndex: n.currentIndex, container: this, item: e });
    }
    _startScrollingIfNecessary(e, t) {
        if (this.autoScrollDisabled)
            return;
        let i, s = m.NONE, n = g.NONE;
        if (this._parentPositions.positions.forEach((o, a) => { a === this._document || !o.clientRect || i || bt(o.clientRect, Tt, e, t) && ([s, n] = de(a, o.clientRect, this._direction, e, t), (s || n) && (i = a)); }), !s && !n) {
    ...
    _canReceive(e, t, i) {
        if (!this._domRect || !j(this._domRect, t, i) || !this.enterPredicate(e, this))
            return !1;
        let s = this._getShadowRoot().elementFromPoint(t, i);
        return s ? s === this._container || this._container.contains(s) : !1;
    }
...
function de(r, e, t, i, s) {
    let n = Vt(e, s), o = Bt(e, i), a = m.NONE, l = g.NONE;
    if (n) {
        let h = r.scrollTop;
...
`], encapsulation: 2, changeDetection: 0 });
    ...
    class r {
        ...
        startDragging(t, i) {
            if (!(this._activeDragInstances().indexOf(t) > -1) && (this._styleLoader.load(_e), this._activeDragInstances.update(s => [...s, t]), this._activeDragInstances().length === 1)) {
    ...
    return r;
})(), pe = { dragStartThreshold: 5, pointerDirectionChangeThreshold: 5 }, st = (() => {
    class r {
        _document = c(Ot);
        _ngZone = c(J);
        _viewportRuler = c(ft);
    ...
    return r;
})(), k = new P("CDK_DRAG_PARENT");
var Gt = new P("CdkDragHandle"), He = (() => {
    class r {
        element = c(Q);
    ...
    return r;
})(), Ut = new P("CDK_DRAG_CONFIG"), Wt = new P("CdkDropList"), ze = (() => {
    class r {
        element = c(Q);
        ...
        _syncInputs(t) {
            ...
            }), t.beforeStarted.pipe(te(1)).subscribe(() => {
                if (this._parentDrag) {
    ...
    class r {
        ...
        removeItem(t) {
            if (this._unsortedItems.delete(t), this._latestSortedRefs) {
        ...
        _setupInputSyncSubscription(t) {
            this._dir && this._dir.change.pipe(At(this._dir.value), N(this._destroyed)).subscribe(i => t.withDirection(i)), t.beforeStarted.subscribe(() => {
                let i = pt(this.connectedTo).map(s => {
                    if (typeof s == "string") {
                ...
                if (this._group && this._group._items.forEach(s => { i.indexOf(s) === -1 && i.push(s); }), !this._scrollableParentsResolved) {
    ...
    return r;
})(), ue = new P("CdkDragPlaceholder"), Ue = (() => {
    class r {
        templateRef = c(Mt);
        _drag = c(k, { optional: !0 });
        data;
