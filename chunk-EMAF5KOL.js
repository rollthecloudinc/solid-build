import { a as m } from "@nf-internal/chunk-XUYTQVBT";
import { c as n } from "@nf-internal/chunk-C2A5IV4Z";
import { b as o, i as _, w as r } from "@nf-internal/chunk-IR6MR6UK";
...
var c = class {
    ...
    focusItem(e, t = {}) {
        t.emitChangeEvent ??= !0;
        let i = typeof e == "number" ? e : this._items.findIndex(I => this._trackByFn(I) === this._trackByFn(e));
        if (i < 0 || i >= this._items.length)
            return;
        let a = this._items[i];
        if (this._activeItem !== null && this._trackByFn(a) === this._trackByFn(this._activeItem))
            return;
        let l = this._activeItem;
        this._activeItem = a ?? null, this._activeItemIndex = i, this._typeahead?.setCurrentSelectedItemIndex(i), this._activeItem?.focus(), l?.unfocus(), t.emitChangeEvent && this.change.next(this._activeItem), this._shouldActivationFollowFocus && this._activateCurrentItem();
    }
    _updateActiveItemIndex(e) {
        let t = this._activeItem;
        if (!t)
            return;
        let i = e.findIndex(a => this._trackByFn(a) === this._trackByFn(t));
    ...
    _expandAllItemsAtCurrentItemLevel() {
        ...
        e ? t = h(e.getChildren()) : t = r(this._items.filter(i => i.getParent() === null)), t.pipe(u(1)).subscribe(i => {
            for (let a of i)
                a.expand();
        });
    }
