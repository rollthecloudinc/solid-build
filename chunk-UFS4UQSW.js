...
import { e as A } from "@nf-internal/chunk-C2A5IV4Z";
import { b as T, w as d, x as M } from "@nf-internal/chunk-IR6MR6UK";
...
var S;
function Y() {
    if (S === void 0 && (S = null, typeof window < "u")) {
        let r = window;
...
var l = class {
    ...
    class r {
        ...
        addSvgIconLiteralInNamespace(t, e, n, o) {
            let i = this._sanitizer.sanitize(g.HTML, n);
            if (!i)
                throw N(n);
            let s = p(i);
        ...
        addSvgIconSetLiteralInNamespace(t, e, n) {
            let o = this._sanitizer.sanitize(g.HTML, e);
            if (!o)
                throw N(e);
            let i = p(o);
            return this._addSvgIconSetConfig(t, new l("", i, n));
        }
        ...
        getSvgIconFromUrl(t) {
            let e = this._sanitizer.sanitize(g.RESOURCE_URL, t);
            if (!e)
                throw y(t);
            let n = this._cachedIconsByUrl.get(e);
            return n ? d(C(n)) : this._loadSvgIconFromConfig(new l(t, null)).pipe(w(o => this._cachedIconsByUrl.set(e, o)), _(o => C(o)));
        }
        getNamedSvgIcon(t, e = "") {
            let n = b(e, t), o = this._svgIconConfigs.get(n);
            if (o)
            ...
            return i ? this._getSvgFromIconSetConfigs(t, i) : M(R(n));
        }
        ...
        _getSvgFromIconSetConfigs(t, e) {
            ...
            if (n)
                return d(n);
            let o = e.filter(i => !i.svgText).map(i => this._loadSvgIconSetFromConfig(i).pipe(j(s => { let m = `Loading icon set URL: ${this._sanitizer.sanitize(g.RESOURCE_URL, i.url)} failed: ${s.message}`; return this._errorHandler.handleError(new Error(m)), d(null); })));
            return A(o).pipe(_(() => {
        ...
        _fetchIcon(t) {
            ...
            let i = this._sanitizer.sanitize(g.RESOURCE_URL, e);
            if (!i)
                throw y(e);
            let s = this._inProgressUrlFetches.get(i);
            if (s)
                return s;
            let u = this._httpClient.get(i, { responseType: "text", withCredentials: o }).pipe(_(m => p(m)), V(() => this._inProgressUrlFetches.delete(i)), B());
            return this._inProgressUrlFetches.set(i, u), u;
        }
...
var H = ["clip-path", "color-profile", "src", "cursor", "fill", "filter", "marker", "marker-start", "marker-mid", "marker-end", "mask", "stroke"], it = H.map(r => `[${r}]`).join(", "), st = /^url\(['"]?#(.*?)['"]?\)$/, Mt = (() => {
    class r {
        ...
        _cacheChildrenWithExternalReferences(t) {
            let e = t.querySelectorAll(it), n = this._elementsWithExternalReferences = this._elementsWithExternalReferences || new Map;
            for (let o = 0; o < e.length; o++)
                H.forEach(i => {
                    let s = e[o], u = s.getAttribute(i), m = u ? u.match(st) : null;
                    if (m) {
                        let v = n.get(s);
